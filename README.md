...
Install signing key:
```
wget -O - http://debian-packages.vrenetic.io/conf/signing.key | apt-key add -
```

Add to your `sources.list`:
```
deb http://debian-packages.vrenetic.io <codename> main
```
...
gpg --export -a 'VRenetic Tech' > /vagrant/keys/public.key
gpg --export-secret-key -a 'VRenetic Tech' > /vagrant/keys/private.key
```
